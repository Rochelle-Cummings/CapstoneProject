...
describe('About Page Left Drop Down Menu First Level Items', () => {
    it('should navigate through drop down menus and check cooresponding links on first level menu', async () => {
        ...
        for (let i = 0; i < AboutPage.arrayAboutMenuNames.length; i++) {
            const menuItem = AboutPage.aboutMenuSelector(AboutPage.arrayAboutMenuNames[i]);
            await menuItem.click();

            const currentUrl = await browser.getUrl();
            await expect(currentUrl).toContain(AboutPage.arrayAboutMenuLinksPaths[i])
            await browser.url('about/');
        }
    })
})

describe('About Page Left Drop Down Menu Secondary Level Items', () => {
    it('should navigate through drop down menus and check corresponding links on submenu options', async () => {
        await browser.url('about/');    
        for (let i = 0; i < AboutPage.arraySecondLevelMenuNames.length; i++) {
            const menuItem = AboutPage.secondLevelMenuSelector(AboutPage.arraySecondLevelMenuNames[i]);
            await menuItem.click();
